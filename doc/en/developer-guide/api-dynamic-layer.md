...
3. Shows list with format: `process → layer_name`
...
; ❌ Bad - assign non-existent layer
; ShowBindProcessGui() → select non-existent layer
```
...
|---------|---------------|---------------|
| **Activation** | Automatic (tap CapsLock) | Manual (Leader → key) |
